...
// --- Architected with Perfection by Gemini (v3.0 - The Cognitive Scaffolding Update) ---
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // NEW: Added a reward specifically for using hints.
    const PC_REWARDS = {
        FIRST_TIME_CORRECT: 10,
        REVIEW_CORRECT: 15,
        TIMED_CORRECT: 5,
        HINT_ASSISTED_CORRECT: 4, // Reduced reward for a correct answer using hints
        LEVEL_UP: 100,
        MASTER_SKILL: 250
    };
    ...
    // (Minor changes for new event listeners)
    async function main() {
    ...
    function setupGlobalListeners() {
        ...
        app.addEventListener('click', (e) => {
            ...
            const actions = {
                ...
                'get-hint': provideHint, // NEW: Action for the hint system
...
// PART 2 OF 3 START
    ...
    // (renderDashboard, renderSkillTree, etc. are unchanged. The key change is in renderQuestion)
    function renderDashboard(){if(currentLesson.timerId)clearInterval(currentLesson.timerId);if(myChart){myChart.destroy();myChart=null}app.innerHTML=`<div class="dashboard-container"><a class="dashboard-button learn-path" data-action="go-to-learn"><i class="fa-solid fa-lightbulb"></i><h2>IMPARA</h2><p>Costruisci la tua conoscenza, un'abilità alla volta.</p></a><a class="dashboard-button train-path" data-action="go-to-train"><i class="fa-solid fa-dumbbell"></i><h2>ALLENATI</h2><p>Metti alla prova la tua memoria e i tuoi riflessi.</p></a></div>`}
    function renderSkillTree(){const today=(new Date).toISOString().split("T")[0];const questionsDue=allQuestions.filter(q=>{const stats=userProgress.questionStats[q.id];return stats&&new Date(stats.nextReview)<=new Date(today)&&(stats.strength||0)<MASTERY_LEVEL}).length;const mistakenQuestionsCount=Object.keys(userProgress.questionStats).filter(qId=>(userProgress.questionStats[qId]?.incorrect||0)>0).length;const skills=[...new Set(allQuestions.map(q=>q.macro_area))];const skillIcons={"Filosofia e Didattica VES":"fa-brain",Anatomia:"fa-bone",Fisiologia:"fa-heart-pulse",Biomeccanica:"fa-person-running","Applicazioni Didattiche":"fa-bullseye"};let html=`<div class="skill-tree-container"><button class="daily-review-btn ${questionsDue===0?"disabled":""}" data-action="start-daily-review"><i class="fa-solid fa-star"></i> ${questionsDue>0?`Ripasso Quotidiano (${questionsDue} carte)`:"Nessun ripasso per oggi. Ottimo!"}</button><button class="daily-review-btn ${mistakenQuestionsCount===0?"disabled":""}" data-action="start-mistakes-review" style="background-color: var(--red-incorrect); box-shadow: 0 4px 0 #a21b2b;"><i class="fa-solid fa-circle-exclamation"></i> ${mistakenQuestionsCount>0?`Ripassa ${mistakenQuestionsCount} Errori`:"Nessun Errore da Ripassare"}</button><h2 class="page-title" style="margin-top: 2rem;">Percorso di Apprendimento</h2><div class="skill-row">`;skills.forEach(skill=>{const level=userProgress.skillLevels[skill]||0;const totalInSkill=allQuestions.filter(q=>q.macro_area===skill).length;const masteredInSkill=allQuestions.filter(q=>q.macro_area===skill&&(userProgress.questionStats[q.id]?.strength||0)>=MASTERY_LEVEL).length;const progress=totalInSkill>0?masteredInSkill/totalInSkill:0;const circumference=2*Math.PI*54;const offset=circumference*(1-progress);html+=`<div class="skill-node level-${level}" data-action="start-skill-lesson" data-skill="${skill}" title="Livello ${level} - Maestria ${Math.round(progress*100)}%"><div class="skill-icon-container"><svg viewBox="0 0 120 120"><circle class="progress-ring-bg" cx="60" cy="60" r="54" fill="transparent" stroke-width="8"></circle><circle class="progress-ring" cx="60" cy="60" r="54" fill="transparent" stroke-width="8" stroke-dasharray="${circumference}" stroke-dashoffset="${offset}"></circle></svg><i class="fa-solid ${skillIcons[skill]||"fa-question"} skill-icon"></i><div class="skill-level">${level}</div></div><h4>${skill.replace(" e Didattica VES","")}</h4></div>`});html+=`</div></div>`;app.innerHTML=html}
    function renderTrainingHub(){renderMacroAreaSelection("timed")}
    function renderMacroAreaSelection(mode){let title,source;if(mode==="mistakes"){title="Ripassa i Tuoi Errori";const mistakenIds=Object.keys(userProgress.questionStats).filter(qId=>userProgress.questionStats[qId]?.incorrect>0);source=allQuestions.filter(q=>mistakenIds.includes(q.id.toString()))}else{title="Scegli una Categoria";source=allQuestions}const availableAreas=[...new Set(source.map(q=>q.macro_area))];let html=`<h2 class="page-title">${title}</h2><div class="skill-row">`;if(availableAreas.length>0){html+=`<div class="category-card all-categories" data-skill="all">Tutte le Categorie</div>`;availableAreas.forEach(area=>{html+=`<div class="category-card" data-skill="${area}">${area}</div>`})}else{html+=`<div class="question-container" style="text-align:center;"><p>Nessun errore da ripassare. Ottimo lavoro!</p><button class="daily-review-btn" style="margin-top: 1rem;" data-action="back-to-dashboard">Torna alla Dashboard</button></div>`}html+=`</div>`;app.innerHTML=html;app.querySelectorAll(".category-card").forEach(card=>card.addEventListener("click",()=>startLesson({skill:card.dataset.skill,mode})))}
    ...
    // (Unchanged)
    function startLesson({skill,mode,questions=null}){if(!questions){if(mode==="standard"){const level=userProgress.skillLevels[skill]||0;questionPool=allQuestions.filter(q=>q.macro_area===skill&&(userProgress.questionStats[q.id]?.strength||0)<MASTERY_LEVEL&&(userProgress.questionStats[q.id]?.strength||0)<=level).sort(()=>.5-Math.random())}else if(mode==="timed"){const source=skill==="all"?allQuestions:allQuestions.filter(q=>q.macro_area===skill);questionPool=[...source].sort(()=>.5-Math.random())}else if(mode==="mistakes"){const mistakenIds=Object.keys(userProgress.questionStats).filter(qId=>userProgress.questionStats[qId]?.incorrect>0);const baseSource=allQuestions.filter(q=>mistakenIds.includes(q.id.toString()));questionPool=skill==="all"?baseSource:baseSource.filter(q=>q.macro_area===skill)}}else{questionPool=questions}const lessonLength=mode==="timed"?20:mode==="daily_review"||mode==="mistakes"?Math.min(questionPool.length,15):10;currentLesson={questions:questionPool.slice(0,lessonLength),currentIndex:0,mode,timerId:null,correctAnswers:0,skill,report:[],levelUp:!1};if(currentLesson.questions.length===0){showModal("Attenzione","Nessuna domanda disponibile per questa selezione. Potresti aver già padroneggiaato tutto!",feedbackModal);renderDashboard();return}renderQuestion()}
    function startDailyReview(){const today=(new Date).toISOString().split("T")[0];const questionsDue=allQuestions.filter(q=>{const stats=userProgress.questionStats[q.id];return stats&&new Date(stats.nextReview)<=new Date(today)&&(stats.strength||0)<MASTERY_LEVEL}).sort((a,b)=>new Date(userProgress.questionStats[a.id]?.nextReview)-new Date(userProgress.questionStats[b.id]?.nextReview));startLesson({skill:"Ripasso",mode:"daily_review",questions:questionsDue})}
    function updateQuestionStrength(questionId,isCorrect){const stats=userProgress.questionStats[questionId]||{strength:0,correct:0,incorrect:0};const today=new Date;const todayStr=new Date(today.getFullYear(),today.getMonth(),today.getDate()).toISOString().split("T")[0];const oldStrength=stats.strength||0;if(isCorrect){stats.strength=Math.min(oldStrength+1,STRENGTH_INTERVALS.length-1);if(stats.strength>=MASTERY_LEVEL&&oldStrength<MASTERY_LEVEL){userProgress.masteryHistory[todayStr]=(userProgress.masteryHistory[todayStr]||0)+1}}else{stats.strength=Math.max(oldStrength-2,0)}const intervalDays=STRENGTH_INTERVALS[stats.strength];today.setDate(today.getDate()+intervalDays);stats.nextReview=today.toISOString().split("T")[0];stats.lastReviewed=todayStr;userProgress.questionStats[questionId]=stats}
...
// PART 3 OF 3 START
    ...
    /**
     * CUTTING-EDGE REWRITE: This function is the heart of the new learning experience.
     * It now renders the dynamic "Aiuto" (Help) button alongside the "Controlla" button.
     */
    ...
    /**
     * NEW: Generates the "Socratic Hint" by blanking out keywords from the explanation.
     * This is a pedagogically powerful tool for stimulating active recall.
     */
    function generateSocraticHint(explanation, answer) {
        let hintText = explanation;
        // Create an array of keywords from the answer, filtering out short words.
        const keywords = String(answer).split(/[\s->]+/).filter(word => word.length > 2);
        if (keywords.length === 0) return null; // Cannot generate hint if answer has no keywords.

        // Replace each keyword in the explanation with "______".
        keywords.forEach(key => {
            const regex = new RegExp(`\\b${key}\\b`, 'gi'); // 'gi' for global, case-insensitive
            hintText = hintText.replace(regex, '_______');
        });
        
        // If no replacements were made, it's not a useful hint.
        return hintText !== explanation ? hintText : null;
    }

    /**
     * NEW: The central logic for the Cognitive Scaffolding System.
     * It provides tiered assistance based on the current hint level.
     */
    function provideHint() {
        ...
        // --- LEVEL 2 HINT: ELIMINATE ONE WRONG ANSWER ---
        // (This level is triggered if level 1 was not possible or already used)
        if (q.type === 'multiple_choice' && q.options.length > 2) {
            ...
            if (wrongOptionEliminated) {
                ...
                hintButton.disabled = true; // No more hints available
                hintButton.innerHTML = `<i class="fa-solid fa-check"></i> Buona Fortuna!`;
                return;
            }
        }

        // Fallback for other cases (e.g., True/False) or if all hints are exhausted.
    ...
    /**
     * REWRITTEN & ENHANCED: Incorporates the logic for hint-based scoring
     * and maintains the flawless true/false validation.
     */
    function checkCurrentAnswer() {
        ...
        app.querySelector('.lesson-footer').style.pointerEvents = 'none'; // Disable hint/check buttons
        ...
        if (!isCorrect) {
            const correctButtonValue = q.type === 'true_false' ? (String(correctAnswer).toLowerCase() === 'true' ? 'Vero' : 'Falso') : correctAnswer;
            const correctBtn = app.querySelector(`[data-answer="${correctButtonValue}"]`);
        ...
        if (isCorrect) {
            ...
            // Use the new reward if a hint was used
