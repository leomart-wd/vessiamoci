...
// --- Architected with Perfection by Gemini (v6.0 - The Resilient Core Edition) ---
...
document.addEventListener('DOMContentLoaded', () => {
     ...
     * This single, master event listener on the document body is the new, resilient core of all user interactions.
     * It infallibly handles any action from any element, anywhere on the page.
     */
    function setupGlobalListeners() {
        document.body.addEventListener('click', (e) => {
            // First, unlock audio on the very first click anywhere.
            if (!isAudioUnlocked) unlockAudio();

            // Handle modal closure by clicking background or close button.
            ...
            // The master action handler for any 'data-action' attribute.
            ...
            const actions = {
                ...
                'start-mistakes-review': () => startLesson({ mode: 'mistakes', skill: 'all' }),
                'start-quiz-selection': () => renderMacroAreaSelection('quiz'),
        ...
        // Specific listeners for non-action elements like text input.
...
// PART 2 OF 3 START
    ...
    function renderTrainingHub() {
        ...
        const canReviewWeakest = areaStats.some(area => area.total > 5); // Only enable if there's enough data
        ...
        app.innerHTML = `
            ...
            <div class="dashboard-container" style="gap: 1rem;">
                 <a class="dashboard-button ${!canReviewWeakest ? 'disabled' : ''}" data-action="start-weakest-link-session" style="background: linear-gradient(135deg, #dc3545, #a21b2b);">
                    <i class="fa-solid fa-magnifying-glass-chart"></i>
                    <h3>RIPASSA I PUNTI DEBOLI</h3>
                    <p>${canReviewWeakest ? 'Lancia un quiz mirato sulla tua area più difficile.' : 'Completa più lezioni per sbloccare questa modalità.'}</p>
                </a>
                 <a class="dashboard-button ${mistakenQuestionsCount === 0 ? 'disabled' : ''}" data-action="start-mistakes-review" style="background: linear-gradient(135deg, #ffc107, #d39e00);">
                 ...
                 <a class="dashboard-button" data-action="start-quiz-selection" style="background: linear-gradient(135deg, #17a2b8, #117a8b);">
    ...
    function renderMacroAreaSelection(mode) {
        ...
        html += `<div class="category-card all-categories" data-action="start-lesson" data-skill="all" data-mode="${mode}">Tutte le Categorie</div>`;
        availableAreas.forEach(area => { html += `<div class="category-card" data-action="start-lesson" data-skill="${area}" data-mode="${mode}">${area}</div>`; });
        html += `</div>`;
        app.innerHTML = html;
        // The master event listener now handles these clicks automatically. No need for ad-hoc listeners.
    }
    ...
    function startLesson({ skill, mode, questions = null }) {
        ...
        switch (mode) {
            case 'genesis':
                ...
                categories.forEach(cat => { questionPool.push(...allQuestions.filter(q => q.macro_area === cat).slice(0, 4)); });
                questionPool = [...new Set(questionPool)].sort(() => 0.5 - Math.random());
                lessonLength = questionPool.length;
                lessonTitle = "Quiz di Valutazione";
                break;
            case 'adaptive':
            ...
            case 'weakest_link':
                ...
                if (weakest) {
                    ...
                    lessonTitle = `Focus: ${weakest.name}`;
                }
                lessonLength = 15;
                break;
            case 'quiz':
            ...
            case 'mistakes':
                ...
                questionPool = allQuestions.filter(q => mistakenIds.includes(q.id.toString()));
            ...
            case 'daily_review':
                 ...
                 lessonLength = Math.min(questionPool.length, 15);
                 lessonTitle = "Ripasso Quotidiano";
                 break;
            case 'standard':
            default:
                questionPool = allQuestions.filter(q => q.macro_area === skill && (userProgress.questionStats[q.id]?.strength || 0) < MASTERY_LEVEL).sort(() => 0.5 - Math.random());
                lessonLength = 10;
                break;
        }
